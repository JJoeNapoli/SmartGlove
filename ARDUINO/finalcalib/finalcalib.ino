...
void ImuSetting(int address_pin) {
  ...
  if (flag == 0) {
    ...
    data = "IMU at Pin " + String(address_pin) + " DETECTED AND CALIBRATED!";
    int length = data.indexOf("!") + 2;
    char data_final[length + 1];
    data.toCharArray(data_final, length + 1);
    ...
    Serial.println("\nProblem with IMU at Pin ");
    Serial.print(address_pin);
    data = "PROBLEM WITH IMU at Pin" + String(address_pin) + "!";
    delay(100);
    int length = data.indexOf("!") + 2;
    char data_final[length + 1];
    data.toCharArray(data_final, length + 1);
...
void readData(int16_t *Ax, int16_t *Ay, int16_t *Az, int16_t *Gx, int16_t *Gy, int16_t *Gz) {
  ...
  Wire.requestFrom(MPU_I2C_ADDR, 14, true);
